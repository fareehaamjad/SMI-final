...
public class KeyExchangeService extends BroadcastReceiver
{
	...
	public void onReceive(Context con, Intent intent) 
	{
		...
		for (TableKeys p: participants)
		{
			if (p.getShouldUpdate_b() && (!p.getexchangeKeysTrying_b()) )
			{
				...
				startExchange(p, dbHandler, encryptionManager);
			}
		}
	...
	private void startExchange(TableKeys p, DatabaseHandler dbHandler, EncryptionManager encryptionManager) 
	{
		p.updateexchangeKeysTrying(true, dbHandler);

		ProcessMessage.sendMetaData(context, dbHandler, encryptionManager);
	}
